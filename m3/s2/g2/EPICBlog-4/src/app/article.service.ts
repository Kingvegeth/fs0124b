...
export class ArticleService {
  ...
  getActiveArticle(): iArticle[] {
    return this.getAllArticle().filter(a => a.active);
  }

  getInactiveArticle(): iArticle[] {
    return this.getAllArticle().filter(a => !a.active);
  }

  getArticlesByTag(tag: string): iArticle[] {
    return this.getAllArticle().filter(a => a.tags.includes(tag));
  }

  getUniqueTags(): string[] {
    const tagList: string[] = [];
    this.getAllArticle().forEach(article => {
